...
export default function Hero() {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault()
    const missingPlate = plate.trim() === ''
    const missingState = !selectedState

    if (missingPlate || missingState) {
      setError(
        missingPlate && missingState
          ? 'Please enter valid plate details.'
          : missingPlate
            ? 'Please enter valid plate details.'
            : 'Please enter valid plate details.'
      )
      if (missingPlate) plateRef.current?.focus()
      return
    }
  ...
  const plateHasError = error?.toLowerCase().includes('plate')
  const stateHasError = error?.toLowerCase().includes('state')
  ...
  return (
    <div className="relative self-stretch bg-[url('/assets/images/HeroBgMobile.jpg')] md:bg-[url('/assets/images/HeroBg.jpg')] bg-cover bg-[position:74%center] md:bg-center bg-no-repeat flex flex-col justify-center items-center gap-7 w-full h-screen">
            ...
            <>
              <div className="w-full md:w-[516px] inline-flex flex-col justify-start items-start gap-1 md:gap-[6px]">
                <div className="self-stretch inline-flex justify-start items-center gap-2 md:gap-4">
                  ...
                  <div className="justify-center text-white text-[18px] md:text-[22px] font-medium leading-[27px]">
                    This vehicle’s not on PL8CHAT... yet
                  </div>
                </div>
                <div className="w-full md:w-[516px] h-48 p-2 bg-stone-600/80 rounded-[28px] flex flex-col justify-start items-start gap-3 md:gap-6">
                  <div className="self-stretch flex flex-col justify-start items-center gap-3 md:gap-4">
                    <div className="self-stretch text-center justify-center text-white text-base font-medium leading-5">
                      Did you know with the free PL8CHAT <br className="md:hidden" /> app you can:
                    </div>
                      ...
                      {Statements.map((item, index) => (
                        <div key={index} className="self-stretch inline-flex justify-start items-end gap-1">
                          <div className="flex justify-start items-center gap-1">
                            <CheckHero />
                            <div className="justify-center text-white text-sm font-light leading-[22px]">
                              {item.statement}
                            </div>
                          </div>
                        </div>
            ...
            <div className="w-full flex flex-col justify-center items-center">
              ...
              <div className='flex flex-col w-full'>
                <form
                  ...
                  className="w-full md:w-auto h-[58px] md:h-[54px] bg-stone-600/80 rounded-[28px] inline-flex justify-start items-center md:gap-2"
                  noValidate
                >
                  <div className="flex justify-between items-center md:gap-10 w-full pr-2 pl-1">
                    <div className="flex flex-row items-center gap-2">
                      ...
                      <input
                        ...
                        maxLength={7}
                        aria-invalid={plateHasError || undefined}
                        aria-describedby={plateHasError ? 'plate-error' : undefined}
                      />
                    </div>

                    {/* If your StateInput can accept a className or error prop, style it similarly */}
                    <StateInput onSelect={(s) => { setSelectedState(s); if (error) setError(null) }} />

                    <Button type="submit" variant="search" className="md:translate-x-[1px]">
                      Search
                    </Button>
                  </div>
                </form>
                <div className="mb-3 w-full md:w-[516px] min-h-[40px]">
                  {error && (
                    <div role="alert" className="flex justify-end text-white text-xs md:text-sm px-4 py-2 text-center">
                      {error}
                    </div>
                  )}
                </div>
      ...
      <div
        className={`relative z-10 w-48 h-10 pt-2 pb-3 rounded-3xl outline outline-1 outline-offset-[-1px] outline-gray-200 inline-flex justify-center items-start gap-3 ${isSearched ? 'hidden' : ''
          }`}
      >
